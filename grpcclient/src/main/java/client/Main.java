...
public class Main {
	...
	static {
		...
		Materia m = Materia.newBuilder().setCodigo("SERVIDOR").setNome("Materia turma servidor")
				.setDescricao("Descricao").build();

		t = Turma.newBuilder().setMateria(m).setProfessor(p).addAllAlunos(lista).build();
	}
